/**
 * Delete All Conversations Script
 * Deletes all conversations and their associated messages from MongoDB
 */
...
async function deleteAllConversations() {
  try {
    ...
    if (connectionUri.includes('localhost') || connectionUri.includes('127.0.0.1')) {
      ...
      // Extract database name from the original URI (after the last / and before ?)
      let dbName = 'syntera'
      try {
      ...
      // Use simple connection string for Docker MongoDB (no auth, no SSL)
      connectionUri = `mongodb://localhost:27017/${dbName}`
      console.log(`   Using Docker MongoDB connection: ${connectionUri}`)
    } else {
      // For remote MongoDB, remove SSL/TLS parameters if present
      connectionUri = connectionUri
        .replace(/[?&]tls=true/gi, '')
        .replace(/[?&]ssl=true/gi, '')
        .replace(/[?&]tlsCAFile=[^&]+/gi, '')
        .replace(/[?&]tlsAllowInvalidCertificates=[^&]+/gi, '')
